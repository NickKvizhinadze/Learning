        ...
        public static List<Passenger> LoadFromFile(string filePath)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var passenger = new Passenger
                {
                    ...
                    Age = decimal.Parse(values[4], NumberStyles.Number, CultureInfo.InvariantCulture),
                    SiblingsOrSpouse = int.Parse(values[5]),
                    ParentOrChildren = int.Parse(values[6]),
                    Fare = decimal.Parse(values[7], NumberStyles.Number, CultureInfo.InvariantCulture)
                };
